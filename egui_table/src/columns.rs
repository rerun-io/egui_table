...
mod tests {
    ...
    fn test_single_column() {
        ...
        auto_size(&mut columns, 50.0);
        assert_eq!(widths(&columns), [100.0]);
        auto_size(&mut columns, 132.0);
        assert_eq!(widths(&columns), [132.0]);
        auto_size(&mut columns, 500.0);
